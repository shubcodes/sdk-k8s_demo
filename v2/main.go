...
func main() {
	...
	if err != nil {
		log.Fatal("ngrok.Listen failed:", err)
	}

	log.Println("Server accessible at:", tun.URL())

	http.HandleFunc("/ws-url", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(tun.URL()))
	})
